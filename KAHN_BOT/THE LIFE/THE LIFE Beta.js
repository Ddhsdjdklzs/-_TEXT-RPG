...
const Rooms = ['디벨봇 테스트', 'ㅡ', 'E', '들어오지마', '110qu3'/*, '팔갈이  시뮬'*/];
...
function responseFix(room, msg, sender, isGroupChat, replier, imageDB, packageName, xxx, Id2){
  ...
  if(msg.startsWith('!가입 ')){
    ...
    Players.unshift({'Name' : DisplayName, 'userId' : Id2, 'Id' : DisplayId, 'PreviousName' : []});

    FS.write(UserPath, JSON.stringify(Players, null, 4));

    FS.write(Path + Id2 + '.txt', JSON.stringify(Data, null, 4));

    replier.reply(acces + DisplayName + '님께서 가입하셨습니다.\'!도움말\'을 입력해 도움을 받아보세요.');
